...
class CocktailRecipeRepository extends BaseRepository
{
    ...
    {
        $sql = "INSERT INTO cocktail_recipes (name, description, instructions, image_url, is_active) 
                 VALUES (:name, :description, :instructions, :image_url, :is_active) 
                 RETURNING *";
    ...
    public function update(int $id, array $data): ?CocktailRecipeModel
    {
        ...
        foreach (['name', 'description', 'instructions', 'image_url', 'is_active'] as $col) {
            if (isset($data[$col])) {
                $sets[] = "$col = :$col";
                $params[":$col"] = $data[$col];
            }
