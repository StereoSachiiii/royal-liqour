...
import { addItemToCart } from '../utils/cart-storage.js';
import { updateCartCount } from '../header/header.js';
import { addItemToWishlist } from '../utils/wishlist-storage.js';
...
const initEventListeners = () => {
    ...
    document.addEventListener('click', async (e) => {
        ...
        if (e.target.closest('.btn-quick-add')) {
            ...
            await addItemToCart(parseInt(btn.dataset.id), 1);
        ...
        if (e.target.closest('#modalAddToCart')) {
            ...
            await addItemToCart(parseInt(btn.dataset.id), qty);
            await updateCartCount();
            closeProductModal();
        }
