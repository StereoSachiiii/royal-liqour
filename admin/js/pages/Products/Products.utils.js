 import { API_URL_PRODUCTS, DETAIL_VIEW_API_URL } from "../config.js";
...
export async function fetchAllProducts(limit = DEFAULT_LIMIT, offset = 0, currentQuery) {
    try {
        const response = await fetch(`${API_URL_PRODUCTS}?limit=${limit}&offset=${offset}&search=${currentQuery}`, {
            method: 'GET',
...
export async function fetchModalDetails(productId) {
    try {
        const response = await fetch(`${DETAIL_VIEW_API_URL}?entity=products&id=${productId}`, {
            method: 'GET',
            credentials: 'include'
        });
        ...
        const data = await response.json();
        return { success: true, product: data.data.data };
