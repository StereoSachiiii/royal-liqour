...
class PaymentController
{
    ...
    {
        error_log(sprintf(
            "[%s] PaymentController Error: %s | File: %s:%d | Context: %s",
            date('Y-m-d H:i:s'),
            $e->getMessage(),
            $e->getFile(),
            $e->getLine(),
    ...
    public function getAll(int $limit = 50, int $offset = 0): array
    {
        return $this->handle(function () use ($limit, $offset) {
            ...
            return $this->success('Payments retrieved', $data);
        });
    }
    ...
    public function getById(int $id): array
    {
        return $this->handle(function () use ($id) {
            ...
            return $this->success('Payment retrieved', $payment->toArray());
        });
    }
    ...
    public function getByOrder(int $orderId): array
    {
        return $this->handle(function () use ($orderId) {
            ...
            $data = array_map(fn($p) => $p->toArray(), $payments);
            return $this->success('Order payments retrieved', $data);
        });
    }
    ...
    public function update(int $id, array $data): array
    {
        return $this->handle(function () use ($id, $data) {
            ...
            return $this->success('Payment updated', $updated->toArray());
        });
    }
