...
class OrderController
{
    ...
    {
        error_log(sprintf(
            "[%s] OrderController Error: %s | File: %s:%d | Context: %s",
            date('Y-m-d H:i:s'),
            $e->getMessage(),
            $e->getFile(),
            $e->getLine(),
    ...
    public function create(array $data): array
    {
        return $this->handle(function () use ($data) {
            ...
            return $this->success('Order created', $order->toArray(), 201);
        });
    }
    ...
    public function getAll(int $limit = 50, int $offset = 0): array
    {
        return $this->handle(function () use ($limit, $offset) {
            ...
            return $this->success('Orders retrieved', $data);
        });
    }
    ...
    public function getByUser(int $userId, int $limit = 50, int $offset = 0): array
    {
        return $this->handle(function () use ($userId, $limit, $offset) {
            ...
            $data = array_map(fn($o) => $o->toArray(), $orders);
            return $this->success('User orders retrieved', $data);
        });
    }
