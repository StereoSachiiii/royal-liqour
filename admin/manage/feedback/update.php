...
<div class="form-group">
    ...
    <script type="module">
      ...
      import { fetchHandler, API_URL, API_FEEDBACK } from '../utils.js';

const recordId = Number(new URLSearchParams(window.location.search).get('id'));

const apiUrl = `${API_URL}?entity=feedback&id=${recordId}`;
...
async function populateUsers() {
    const res = await fetchHandler(`${API_URL}?entity=users&limit=500`, "GET");
...
async function populateProducts() {
    const res = await fetchHandler(`${API_URL}?entity=products&limit=500`, "GET");
