...
<body>
    ...
    <script type="module">
        import { fetchHandler, API_URL, API_PRODUCT_RECOGNITION } from '../utils.js';

        const recordId = Number(new URLSearchParams(window.location.search).get('id'));
        const apiUrl = `${API_URL}?entity=product_recognition&id=${recordId}`;
        ...
        async function loadUsers() {
            try {
                const response = await fetchHandler(`${API_URL}?entity=users`, 'GET');
        ...
        async function loadProducts() {
            try {
                const response = await fetchHandler(`${API_URL}?entity=products`, 'GET');
        ...
        const loadProductRecognitionData = async () => {
            const data = await fetchHandler(`${API_URL}?entity=product_recognition&id=${recordId}`, 'GET');
