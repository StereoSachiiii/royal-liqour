...
<body>
    ...
    <script type="module">
        import { API_URL, API_RECIPE_INGREDIENT } from '../utils.js';
        ...
        const populateDatalists = async () => {
            const recipes = await doFetch(`${API_URL}?entity=recipe_ingredients`);
            ...
            const products = await doFetch(`${API_URL}?entity=products`);
        ...
        const loadData = async () => {
            const res = await doFetch(`${API_URL}?entity=recipe_ingredients&id=${recordId}`);
            if (!res?.data) {
